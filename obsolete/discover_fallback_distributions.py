...
class KLDivergenceCaluclator:
    def __init__(self, olmo_predictions_path):
        self.olmo_predictions_path = olmo_predictions_path
        
        # Extract model identifier from path for plot naming
        if "suzeva_olmo2-1b-4xH100-2ndtry" in olmo_predictions_path:
            self.model_id = "suzeva_olmo2-1b-4xH100-2ndtry"
        elif "allenai_OLMo-2-0425-1B-stage1-step10000-tokens21B" in olmo_predictions_path:
            self.model_id = "allenai_OLMo-2-0425-1B-stage1-step10000-tokens21B"
        else:
            # Extract a reasonable identifier from the path
            path_parts = olmo_predictions_path.split("__")
            if len(path_parts) >= 3:
                self.model_id = path_parts[2].split("__")[0]  # Take the model part
            else:
        ...
        with open(self.olmo_predictions_path, "r") as f:
    ...
    def calculate_kl_divergence(self, training_data_distributions):
        ...
        # Overall KL divergence across all years (ID + OOD)
        all_years = set(range(1000, 3000))
        overall_avg_kl, overall_years_used, overall_year_to_kl = self._calculate_kl_divergence_for_years(all_years, training_data_distributions)
        ...
        return {
            ...
            'overall_avg_kl': overall_avg_kl,
            'overall_years_used': overall_years_used,
            'overall_year_to_kl': overall_year_to_kl
        }
    ...
    def co_occurrence_fallback(self, plot=False):
        ...
        for year in relative_counts.keys():
            ...
            total_occurrences += total_be_tense_counts
        ...
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_co_occurrence_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                year_to_relative_counts, 
                title=f"KL Divergence by Year: Co-occurrence Fallback vs {self.model_id}",
                save_path=save_path
            )
        
        results = self.calculate_kl_divergence(year_to_relative_counts)
        results['total_occurrences'] = total_occurrences
        return results

    def avg_co_occurrence_fallback(self, plot=False):
        with open(OLMO_TRAINING_DATA_FILE, "r") as f:
            olmo_training_data = json.load(f)
        ...
        for year in relative_counts.keys():
            counts_per_year = relative_counts[year]
            past_count = counts_per_year.get("was", 0) + counts_per_year.get("were", 0)
            present_count = counts_per_year.get("is", 0) + counts_per_year.get("are", 0)
            future_count = counts_per_year.get("will", 0)
            
            avg_relative_counts["past"] += past_count
            avg_relative_counts["present"] += present_count
            avg_relative_counts["future"] += future_count
            total_occurrences += past_count + present_count + future_count

        avg_year_to_relative_counts = {year:avg_relative_counts for year in range(1000, 3000)}
        
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_avg_co_occurrence_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                avg_year_to_relative_counts, 
                title=f"KL Divergence by Year: Average Co-occurrence Fallback vs {self.model_id}",
                save_path=save_path
            )
        
        results = self.calculate_kl_divergence(avg_year_to_relative_counts)
        results['total_occurrences'] = total_occurrences
        return results


    def co_occurrence_and_string_match_fallback(self, plot=False):
        ...
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_co_occurrence_and_string_match_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                year_to_relative_counts, 
                title=f"KL Divergence by Year: Co-occurrence and String Match Fallback vs {self.model_id}",
                save_path=save_path
            )
    ...
    def avg_co_occurrence_and_string_match_fallback(self, plot=False):
        with open(OLMO_EXTRA_TRAINING_DATA_FILE, "r") as f:
            olmo_training_data = json.load(f)
        ...
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_avg_co_occurrence_and_string_match_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                avg_year_to_relative_counts, 
                title=f"KL Divergence by Year: Average Co-occurrence and String Match Fallback vs {self.model_id}",
                save_path=save_path
            )
    ...
    def exact_string_matching_fallback(self, plot=False):
        with open(OLMO_TRAINING_DATA_FILE, "r") as f:
            olmo_training_data = json.load(f)
        ...
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_exact_string_matching_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                year_to_relative_counts, 
                title=f"KL Divergence by Year: Exact String Matching Fallback vs {self.model_id}",
                save_path=save_path
            )
    ...
    def avg_exact_string_matching_fallback(self, plot=False):
        with open(OLMO_TRAINING_DATA_FILE, "r") as f:
            olmo_training_data = json.load(f)
        ...
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_avg_exact_string_matching_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                avg_year_to_relative_counts, 
                title=f"KL Divergence by Year: Average Exact String Matching Fallback vs {self.model_id}",
                save_path=save_path
            )
    ...
    def uniform_distribution_fallback(self, plot=False):
        year_to_relative_counts = {year:{"past":1/3, "present":1/3, "future":1/3} for year in range(1000, 3000)}
        
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_uniform_distribution_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                year_to_relative_counts, 
                title=f"KL Divergence by Year: Uniform Distribution Fallback vs {self.model_id}",
                save_path=save_path
            )
        
        results = self.calculate_kl_divergence(year_to_relative_counts)
        results['total_occurrences'] = "N/A (uniform distribution)"
        return results

    def be_verb_fallback(self, plot=False):
        with open(OLMO_TRAINING_DATA_FILE, "r") as f:
            olmo_training_data = json.load(f)

        tense_counts = olmo_training_data["[~tense~]_counts"]
        total_occurrences = tense_counts["was"] + tense_counts["were"] + tense_counts["is"] + tense_counts["are"] + tense_counts["will"]
        ...
        if plot:
            model_suffix = self.model_id.replace("-", "_").replace(".", "_")
            save_path = f"kl_divergence_be_verb_fallback_{model_suffix}.png"
            self.plot_kl_divergence_by_year(
                year_to_relative_counts, 
                title=f"KL Divergence by Year: Be Verb Fallback vs {self.model_id}",
                save_path=save_path
            )
...
def run_all_fallbacks(kl_div, plot=True):
    results = kl_div.co_occurrence_fallback(plot=plot)
    ...
    print(f"Overall Average KL: {results['overall_avg_kl']}, Years used: {len(results['overall_years_used'])}")

    results = kl_div.avg_co_occurrence_fallback(plot=plot)
    ...
    results = kl_div.exact_string_matching_fallback(plot=plot)
    ...
    results = kl_div.avg_exact_string_matching_fallback(plot=plot)
    ...
    results = kl_div.uniform_distribution_fallback(plot=plot)
    ...
    results = kl_div.be_verb_fallback(plot=plot)
    print("\n\n be_verb_fallback")
    print(f"Total occurrences: {results['total_occurrences']}")
    print(f"ID Average KL: {results['id_avg_kl']}, Years used: {len(results['id_years_used'])}")
    print(f"OOD Average KL: {results['ood_avg_kl']}, Years used: {len(results['ood_years_used'])}")
    ...
    results = kl_div.co_occurrence_and_string_match_fallback(plot=plot)
    ...
    results = kl_div.avg_co_occurrence_and_string_match_fallback(plot=plot)
...
if __name__ == "__main__":
    ...
    # Default to suzeva model, but can be changed
    suzeva_olmo_predictions_path = "../olmo_predictions/1000-3000__was.were.is.are.will__suzeva_olmo2-1b-4xH100-2ndtry-step-10000__In_[year]_there/olmo_predictions.json"
    official_olmo_predictions_path = "../olmo_predictions/1000-3000__was.were.is.are.will__allenai_OLMo-2-0425-1B-stage1-step10000-tokens21B__In_[year]_there/olmo_predictions.json"
    
    print("SUZEVA")
    kl_div = KLDivergenceCaluclator(suzeva_olmo_predictions_path)
    run_all_fallbacks(kl_div)

    print("OFFICIAL")
    kl_div = KLDivergenceCaluclator(official_olmo_predictions_path)
    run_all_fallbacks(kl_div)
