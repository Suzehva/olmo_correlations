...
def kl_divergence_from_dicts(dist_p, dist_q, epsilon=1e-12):
    ...
    """Calculate KL divergence KL(Model || Training Data) between two distributions from dictionaries."""
...
def plot_kl_divergence(years, kl_values, olmo_training_data, data_source="in_year_there_word_counts", 
    ...
    relative_counts = olmo_training_data[data_source]
    for y, kl in zip(years, kl_values):
        if kl is not None:
            filtered_years.append(int(y))
            filtered_kl.append(kl)
            # total count for coloring
            counts_per_year = relative_counts[y]
            ...
            filtered_counts.append(total_count)

    years_int = filtered_years
    ...
    years_int = [int(year) for year in years] # added bc the training data doesnt have year 3000
...
if __name__ == "__main__":
    ...
    for year in sorted(year_to_relative_predictions.keys()):
        ...
        if year in year_to_relative_counts:
    ...
    kl_values_filtered = [kl for kl in kl_values if kl is not None]
    print(f"\nAverage KL divergence (training || model): {np.mean(kl_values_filtered):.4f}")
    print(f"Number of years with KL values: {len(kl_values_filtered)}")
